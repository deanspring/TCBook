...
public class PersonLikeMusicalArtistDAOImpl extends DAO implements PersonLikeMusicalArtistDAO {
    ...
    public Map<Long, Float> averageByArtist() {
        ...
        try {
            final StringBuilder sb = new StringBuilder();
            sb.append("SELECT id_artista_musical, AVG(nota) AS media FROM PessoaCurteArtistaMusical " +
    ...
    public Map<Long, Integer> topTenPopularArtists() {
        ...
        try {
            final StringBuilder sb = new StringBuilder();
            sb.append("SELECT id_artista_musical, COUNT(1) AS ocorrencias FROM PessoaCurteArtistaMusical " +
                    "GROUP BY id_artista_musical " +
    ...
    public Map<Long, Float> topTenStandardDeviationByArtist() {
        ...
        try {
            final StringBuilder sb = new StringBuilder();
            sb.append("SELECT id_artista_musical, STD(nota) AS desvio FROM PessoaCurteArtistaMusical " +
                    "GROUP BY id_artista_musical " +
                    "HAVING COUNT(1) >= 2 " +
                    "ORDER BY STD(nota) DESC " +
                    "LIMIT 10;");
    ...
    public Map<Integer, Integer> peopleLikesByAmount() {
        ...
        try {
            ...
            sb.append("SELECT tmp.ocorrencias as quantas_curtidas, count(1) as ocorrencias FROM " +
                    "(SELECT COUNT(1) AS ocorrencias,id_pessoa FROM PessoaCurteArtistaMusical GROUP BY id_Pessoa) AS tmp " +
                    "GROUP BY quantas_curtidas " +
    ...
    public Map<Integer, Integer> artistsLikesByAmount() {
        ...
        try {
            final StringBuilder sb = new StringBuilder();
            sb.append("SELECT tmp.ocorrencias as quantas_curtidas, count(1) as ocorrencias FROM " +
                    "(SELECT COUNT(1) as ocorrencias,id_artista_musical FROM PessoaCurteArtistaMusical GROUP BY id_artista_musical) AS tmp " +
                    "GROUP BY quantas_curtidas " +
                    "ORDER BY quantas_curtidas;");
